...
char_count = $0003 ; Keep track of line length
...
reset:
  ...
  sta kb_wptr
  sta kb_rptr
  sta char_count   ; Initialize to 0
...
do_backspace:
  lda char_count
  beq skip_bs            ; If no chars printed, do nothing
  dec char_count

  ; 1) Move cursor left (HD44780 "Move cursor left" command = $10)
  lda #$10
  jsr lcd_instruction

  ; 2) Print space to overwrite old character
  lda #' '
  jsr print_char

  ; 3) Move cursor left again so we end up back on the erased position
  lda #$10
  jsr lcd_instruction
...
print_char:
  ...
  eor #E          ; Clear E bit
  sta PORTB
  ; After sending the character, increment char_count and check for wrap
  lda char_count
  inc
  sta char_count
  cmp #16         ; have we printed 16 chars on current line?
  bne done
  lda #%11000000  ; if yes, move cursor to second line (DDRAM address $40)
  jsr lcd_instruction
done:
  rts
